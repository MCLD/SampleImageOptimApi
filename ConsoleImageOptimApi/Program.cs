    ...
    internal static class Program
    {
        ...
        private static async Task<int> Main(string[] args)
        {
            var title = Assembly
                ...
                .Title ?? nameof(ConsoleImageOptimApi);
            ...
            {
                Console.Error.WriteLine($"An error occurred: {oex.Message}");
                return 1;
            }
            catch (Exception ex)
            {
        ...
        private static async Task<int> RunOptionsAsync(Options options)
        {
            ...
            try
            {
                ...
                if (options.Debug)
                {
                    ...
                    {
                        Console.WriteLine($"Server header: {result.ServerHeader}");
                    }

                    if (!string.IsNullOrEmpty(result.ViaHeader))
                    {
                        Console.WriteLine($"Via header: {result.ViaHeader}");
                    }
                ...
                if (result.Status == Status.Success)
                {
                    if (result.File?.Length > 0)
                    {
                        ...
                        {
                            Console.WriteLine($"Optimized {inputFilename} to {outputFilename} {result.File.Length:N0} bytes in {result.ElapsedSeconds:N0}s");
                        }
                        else
                        {
                            Console.WriteLine($"Optimized {inputFilename} to {outputFilename} from {result.OriginalSize:N0} to {result.File.Length:N0} bytes in {result.ElapsedSeconds:N0}s");
                        }
                    ...
                    {
                        Console.Error.WriteLine("Returned file is empty.");
                    }
                }
                else
                {
                    Console.Error.WriteLine($"Status: {result.Status}");
                    if (result.StatusMessage != null)
                    {
                        Console.Error.WriteLine($"Status details: {result.StatusMessage}");
                    }
                }
            }
            catch (ParameterException pex)
            {
                Console.Error.WriteLine($"Parameter parsing exception: {pex.Message}");
            }
            return 0;
        }
    }
